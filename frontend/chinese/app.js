...
function updateStepTitle(step) {
  ...
  if (progressSection && step > 1) {
    const stepTexts = {
      2: { number: '步骤 2', description: '预览您的记录' },
      3: { number: '步骤 3', description: '提交到区块链' },
      4: { number: '步骤 4', description: '完成！' }
    };
...
async function connectToWallet(walletProvider, walletInfo) {
  try {
    ...
    showError(`连接${walletInfo.name}时出错：${error.message}`);
...
function validateStep1() {
  ...
  if (!entryTitle || !story) {
    alert('请填写所有必填字段（标题和故事）。图片和标签是可选的。');
    return false;
  }
  
  // Check character limit for story
  if (story.length > 280) {
    alert('您的消息太长了！请保持在280字符以内。');
    return false;
  }
  ...
  if (sanitizedEntryTitle !== entryTitle || 
    ...
    alert('在输入中检测到无效字符。请删除任何HTML标签或脚本内容。');
    return false;
  }
