...
// =============  CONFIGURATION  =============
// Network configuration moved to public config files
...
function updateStepTitle(step) {
  ...
  if (progressSection && step > 1) {
    const stepTexts = {
      2: { number: 'Paso 2', description: 'Vista previa de tu entrada' },
      3: { number: 'Paso 3', description: 'Enviar a blockchain' },
      4: { number: 'Paso 4', description: '¡Completado!' }
    };
...
async function connectToWallet(walletProvider, walletInfo) {
  try {
    ...
    showError(`Error al conectar con ${walletInfo.name}: ${error.message}`);
...
function validateStep1() {
  ...
  if (!entryTitle || !story) {
    alert('Por favor completa todos los campos requeridos (título y historia). La imagen y etiquetas son opcionales.');
    return false;
  }
  
  // Check character limit for story
  if (story.length > 280) {
    alert('¡Tu mensaje es demasiado largo! Por favor manténlo bajo 280 caracteres.');
    return false;
  }
  ...
  if (sanitizedEntryTitle !== entryTitle || 
    ...
    alert('Se detectaron caracteres inválidos en la entrada. Por favor elimina cualquier etiqueta HTML o contenido de script.');
    return false;
  }
...
async function startEncryptionInBackground() {
  ...
  try {
    ...
    document.getElementById('preview-encryption-progress').style.width = '50%';
      // 3. Encrypt story
    document.getElementById('preview-encryption-status').textContent = 'Encriptando historia...';
    ...
    document.getElementById('preview-encryption-progress').style.width = '70%';
      // 4. Encrypt image
    document.getElementById('preview-encryption-status').textContent = 'Encriptando imagen...';
    ...
    // 5. Upload to IPFS
    document.getElementById('preview-encryption-status').textContent = 'Subiendo a IPFS...';
...
async function submitToChain() {
  ...
  try {
    ...
    if (submissionStatus) submissionStatus.textContent = 'Preparando transacción...';
    ...
    if (submissionStatus) submissionStatus.textContent = '¡Transacción enviada! Esperando confirmación...';
